// Copied verbatim from frontend/src/data/exercises.ts (values unchanged) so the
// seed can't drift from what the UI actually shows. Only the import — a path
// alias that doesn't resolve from the backend project — was swapped for a
// local type mirroring frontend/src/types/workout.ts's exact string unions.
// 71 entries total (the original frontend audit's "76" was a miscount —
// verified here with `grep -c "^  { id:"` against the source file).
type Equipment =
  | 'Dumbbell'
  | 'Kettlebell'
  | 'Barbell / Rod'
